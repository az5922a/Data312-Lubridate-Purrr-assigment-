...
sba_investments |> distinct(LoanStatus)
...
sba_investments |> filter(BorrState == "DC" , NaicsCode == 238150)
...
sba_jobs_by_state <- sba_subset |> 
  group_by(BorrState) |> 
  summarize(total_jobs = sum(JobsSupported, na.rm = TRUE))


head(sba_jobs_by_state)
```
...
sba_interest_by_state <- sba_subset |> 
  group_by(BorrState) |> 
  summarize(average_interest = mean(InitialInterestRate, na.rm = TRUE))


head(sba_interest_by_state) 
...
sba_median_approval <- sba_subset |> 
...
sba_nail_salon_va_md <- sba_subset |> 
  filter(NaicsDescription == "Nail Salons") |> 
  filter(BorrState %in% c("VA", "MD")) |> 
  summarize(sum_approved = sum(GrossApproval, na.rm = TRUE))

head(sba_nail_salon_va_md)
...
| Full-Service Restaurants                                           | 13476130                               |
| Limited-Service Restaurants                                        | 89105000                               |
...
top_5_industries <- sba_subset |> 
  group_by(NaicsDescription) |> 
  summarize(total_loan_approved = sum(GrossApproval, na.rm = TRUE)) |> 
  arrange(desc(total_loan_approved)) |> 
  slice_head(n = 5)
...
sba_subset_v2 <- sba_subset |> 
   rename(program_name = Program, borrower_name = BorrName, borrower_state = BorrState,
   loan_approved = GrossApproval, interest_rate = InitialInterestRate, industry_description =     NaicsDescription,jobs_supported = JobsSupported )
